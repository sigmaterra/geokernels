"""Geodesic kernels for Gaussian process regression and classification.
...
The following kernels are implemented for geodesic regression and classification, 
and added to default kernels as geodesic kernel versions:
...
The geodesic kernels are build for the use-case that the spatial dimensions are given
as Latitude and Longitude in the WGS84 coordinate system 
(i.e., the default Latitude/Longitude system in degrees). 

The geodesic kernels can be called with data X, where X has the shape (n_samples, n_features),
and the condition that the first two features (dimensions) are given by the latitude (1st dimension) 
and longitude (2nd dimension) coordinates.

The geodesic distance is computed using Vincenty's inverse method formula.

Both, anisotropic and isotropic kernels are supported.

For an isotropic kernel, the length scale is a scalar, and the distance 
is calculated as mix geodesic distance and euclidean distance (for remaining features). 

For an anisotropic kernel, the length scale is given as list of length of features minus one.
The length_scale list has one less element than the number of features because the first two dimensions
(Latitude and Longitude) are spatial dimensions and are combined into one geodesic distance metric.
...
Note: this module requires sklearn.
"""
...
def _check_length_scale_geodesic(X, length_scale):
    ...
    if np.ndim(length_scale) > 1:
        raise ValueError("length_scale cannot be of dimension greater than 1")
    if np.ndim(length_scale) == 1 and X.shape[1] != length_scale.shape[0] + 1:
        raise ValueError(
            "Anisotropic kernel must equal the number of "
            "dimensions as data (%d!=%d) minus 1" % (length_scale.shape[0], X.shape[1])
        )
    if abs(X[:,0]).any() > 90:
        raise ValueError("Dimension X[:,0] must be Latitude in degrees (-90 <= X[:,0] <= 90)")
    if abs(X[:,1]).any() > 180:
        raise ValueError("Dimension X[:,1] must be Longitude in degrees (-180 <= X[:,1] <= 180)")
    return length_scale
...
class RBF_geo(StationaryKernelMixin, NormalizedKernelMixin, Kernel):
    """Radial-basis function kernel (aka "squared exponential" kernel) 
    with geodesic distance metric (for spatial coordinates) and 
    Euclidian distance metric for all other features.

    This kernel assumes that the first two dimensions of X are given 
    as Latitude (1st dimension) and Longitude (2nd dimension) in degrees.

    The RBF_geo kernel is a modified version of the stationary kernel RBF and 
    includes geodesic distance calculation in meters. 
    
    It is also known as the "squared exponential" kernel. It is parameterized by a length scale
    parameter :math:`l>0`, which can either be a scalar (isotropic variant
    of the kernel) or a vector with the number of dimensions as the inputs
    X minus one (anisotropic variant of the kernel minus combined spatial lenghtscale). 
    ...
    where :math:`l` is the length scale of the kernel and
    :math:`d(\\cdot,\\cdot)` is the geodesic distance for first two dimensions 
    (latitude, longitude) and Euclidean distance for remaining dimension.

    The length_scale parameter can be set either as scalar (same lengthscale for 
    all dimensions) or as list of length equal to the number of dimensions minus one. 
    The length_scale list has one less element than the number of features because the 
    first two dimensions (Latitude and Longitude) are spatial dimensions and are combined 
    in one geodesic distance metric.

    The gradient computation for each data pair and dimension is based on Vincenty's inverse method 
    
    This kernel is infinitely differentiable, which implies that GPs with this
    kernel as covariance function have mean square derivatives of all orders,
    and are thus very smooth.

    For further details on the RBF see documentation on the RBF kernel.

    .. versionadded:: 1.1.1

    Parameters
    ----------
    length_scale : float or ndarray of shape (n_features - 1,), default=1.0
        The length scale of the kernel. If a float, an isotropic kernel is
        used. If an array, an anisotropic kernel is used where each dimension
        of l defines the length-scale of the respective feature dimension. 
        The first lengthscale defines the spatial lengthscale for 
        the geodesic distance metric based on the first two dimensions 
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        X = np.atleast_2d(X)
        length_scale = _check_length_scale_geodesic(X, self.length_scale)
        if np.size(length_scale) == 1:
            lengthscales = np.ones(X.shape[1] - 1) * length_scale
        else:
            lengthscales = length_scale
        if Y is None:
            dists = (
                pdist(X[:,:2], metric = lambda u, v: geodesic_vincenty(u, v))**2 / lengthscales[0]**2 + 
                pdist(X[:,2:] / lengthscales[1:],  metric = 'sqeuclidean')
            )
            K = np.exp(-0.5 * dists)
            # convert from upper-triangular matrix to square matrix
            K = squareform(K)
            ...
            if eval_gradient:
                raise ValueError("Gradient can only be evaluated when Y is None.")
            dists = (
                    cdist(X[:,:2], Y[:,:2], metric = lambda u, v: geodesic_vincenty(u, v))**2 / lengthscales[0]**2 +
                        cdist(X[:,2:] / lengthscales[1:], Y[:,2:] / lengthscales[1:],  metric = 'sqeuclidean')
                )
            K = np.exp(-0.5 * dists)
        ...
        if eval_gradient:
            if self.hyperparameter_length_scale.fixed:
                ...
                # We need to recompute the pairwise dimension-wise distances
                K_gradient = geodist_dimwise(X)**2 / length_scale**2
                K_gradient *= K[..., np.newaxis]
                return K, K_gradient
        else:
            return K
...
class Matern_geo(RBF):
    """ Matern kernel with geodesic distance metric (for spatial coordinates) and 
    Euclidian distance metric for all other features.

    This kernel assumes that the first two dimensions of X are given 
    as Latitude (1st dimension) and Longitude (2nd dimension) in degrees.

    The Matern_geo kernel is a modified version of the stationary kernel Matern and 
    includes geodesic distance calculation in meters. 
    ...
    where :math:`d(\\cdot,\\cdot)` is the geodesic distance for first two dimensions (latitude, longitude),
    and Euclidian distance for all other dimensions,
    :math:`K_{\\nu}(\\cdot)` is a modified Bessel function and
    :math:`\\Gamma(\\cdot)` is the gamma function.
    See [1]_, Chapter 4, Section 4.2, for details regarding the different
    variants of the Matern kernel.
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        length_scale = _check_length_scale_geodesic(X, self.length_scale)
        if np.size(length_scale) == 1:
            lengthscales = np.ones(X.shape[1]) * length_scale
        else:
            lengthscales = length_scale
        if Y is None:
            dists = (
                pdist(X[:,:2], metric = lambda u, v: geodesic_vincenty(u, v)) / lengthscales[0] + 
                pdist(X[:,2:] / lengthscales[1:],  metric = 'euclidean')
            )
        else:
            if eval_gradient:
                raise ValueError("Gradient can only be evaluated when Y is None.")
            dists = (
                    cdist(X[:,:2], Y[:,:2], metric = lambda u, v: geodesic_vincenty(u, v)) / lengthscales[0] +
                        cdist(X[:,2:] / lengthscales[1:], Y[:,2:] / lengthscales[1:],  metric = 'euclidean')
                )
        ...
        if eval_gradient:
            ...
            # We need to recompute the pairwise dimension-wise distances
            if self.anisotropic:
                D = geodist_dimwise(X)**2 / length_scale**2
            else:
                D = squareform(dists**2)[:, :, np.newaxis]
...
class RationalQuadratic_geo(StationaryKernelMixin, NormalizedKernelMixin, Kernel):
    """Rational Quadratic kernel with geodesic distance metric (for spatial coordinates) and 
    Euclidian distance metric for all other features.

    This kernel assumes that the first two dimensions of X are given 
    as Latitude (1st dimension) and Longitude (2nd dimension) in degrees (WGS84).

    The RationalQuadratic_geo kernel is a modified version of the stationary kernel RationalQuadratic
    and includes geodesic distance calculation in meters. 
    ...
    where :math:`\\alpha` is the scale mixture parameter, :math:`l` is
    the length scale of the kernel and :math:`d(\\cdot,\\cdot)` is the 
    geodesic distance for first two dimensions (latitude, longitude),
    and Euclidian distance for all other dimensions.
    For advice on how to set the parameters, see e.g. [1]_.
